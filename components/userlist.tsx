...
const UserListComp = ({ name, studentId, secondary, Class, classNumber, status, total, editStudent }: any) => {
    ...
    return (
        <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" key={studentId}>
            ...
            <td className="px-10 py-4" >{classNumber}</td>
            <td className="px-9 py-4" >{studentId}</td>
            <td className="px-0.5 py-4" >{status === false ? "🟢 Not Leave" : "🔴 Left"}</td>
            <td className="px-5 py-4" >{total} Days</td>
